/* Runtime dump - SettingsAccessibilityCaptioningColorController
 * Image: /Applications/AppleTV.app/AppleTV
 */


#import <BackRow/SettingsAccessibilityCaptioningStyleBaseController.h>

@class BRControl, BRListControl, BRMenuItem;
@interface SettingsAccessibilityCaptioningColorController : SettingsAccessibilityCaptioningStyleBaseController
{
    int _colorType;
    struct CGColor * _selectedColor;
}

- (BRMenuItem *)list:(BRListControl *)arg0 menuItemForRowAtIndexPath:(NSIndexPath *)arg1;
- (long)numberOfSectionsInList:(BRListControl *)arg0;
- (long)list:(BRListControl *)arg0 numberOfRowsInSection:(long)arg1;
- (BRControl *)list:(BRListControl *)arg0 sectionHeaderForSection:(long)arg1;
- (void)list:(BRListControl *)arg0 didSelectItemAtIndexPath:(NSIndexPath *)arg1;
- (void)_reloadList;
- (struct CGColor *)_colorRefForCaptionColor:(long)arg0;
- (void)_toggleVideoOverridesStyle;
- (SettingsAccessibilityCaptioningColorController *)initWithTitle:(NSString *)arg0 andProfileID:(NSString *)arg1 andColorType:(int)arg2;
- (char)_videoOverridesStyle;
- (void)dealloc;

@end
